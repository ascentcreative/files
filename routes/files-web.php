...
use AscentCreative\Images\Models\Image as Image;
use AscentCreative\Images\Models\OutputSpec;

use AscentCreative\Images\ImageSizer;


Route::middleware(['web'])->group(function() {
    
    Route::post('/image-upload', function($spec=null) {
        ...
        // Store image on disk.
        $path = Storage::disk('images')->putFile('original', request()->file('payload'));
  
        // Create an Image model, but don't save it? Return as JSON? 
        $image = new Image();
        $image->disk = 'images';
        $image->hashed_filename = pathinfo($path)['basename']; // only store the hashed name in the model. Don't need any folders etc.

        $payload = request()->file('payload');
        $sanitise = $payload->getClientOriginalName();
        $sanitise = str_replace(array('?', '#', '/', '\\', ','), '', $sanitise);
        $image->original_filename = $sanitise; // nb - the Image model will check for duplicate filenames and increment on save.
